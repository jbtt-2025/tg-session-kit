...
- 可选（心跳相关）：`TG_INTERVAL_SECONDS`（默认 1 天）、`TG_JITTER_SECONDS`（默认 300s）、`TG_MODE`
- 可选（通知）：`TG_NOTIFY_BOT_TOKEN`、`TG_NOTIFY_CHAT_ID`（启用后每次保活成功/失败会通过 Bot API 推送）
...
- heartbeat 通知：设置 `TG_NOTIFY_BOT_TOKEN` 与 `TG_NOTIFY_CHAT_ID` 后，每次保活成功/失败会通过 Bot API 发送 `{chat_id} 保活成功/失败 {reason}`。
